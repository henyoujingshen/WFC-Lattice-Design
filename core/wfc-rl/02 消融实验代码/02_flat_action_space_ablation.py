...
class WFC_RL_Net_Flat(nn.Module):
    """
    一个无条件输入的、单输出头的扁平化动作空间网络。

    该网络仅将网格的当前状态作为输入，输出一个覆盖所有位置和所有
    模块的单一概率分布。它作为“两步动作”模型的对照组。
    """
    ...
    def __init__(self, grid_height, grid_width, num_states,
        ...
        # 单一的输出头，直接输出所有动作的 logits
        self.output_conv = nn.Sequential(
            nn.Conv2d(fused_feature_dim, fused_feature_dim // 2, kernel_size=3, padding=1),
            nn.BatchNorm2d(fused_feature_dim // 2), nn.GELU(),
            nn.Conv2d(fused_feature_dim // 2, num_states, kernel_size=1)
        )
    ...
    def forward(self, grid_constraints):
        ...
        # 3. 通过输出头得到 logits
        logits = self.output_conv(fused_output)
        logits_permuted = logits.permute(0, 2, 3, 1)  # 形状: (B, H, W, S)

        # 4. 返回原始 logits，让动作选择函数来处理 softmax
        #    这样可以更灵活地应用 mask
        return logits_permuted
...
def run_batched_episodes(network, fiber_block_library, goal_K_tensor, initial_grid,
    ...
    for step in range(max_steps_per_episode):
        ...
        # in run_batched_episodes()

        # ...
        # 1. 网络只返回一个包含所有 logits 的张量
        all_logits_batch = network(batch_constraints)  # 形状: (B, H, W, S)

        # 从 history 中移除，因为我们不再有单独的 state_probs
        # if all_logits_batch.shape[0] > 0:
        #    batch_state_probs_history.append(...) # 需要调整损失函数或移除此行

        # 2. 在循环中，使用新的动作选择函数
        for i, env_idx in enumerate(active_indices):
            grid = grids[env_idx]
            # 提取当前环境的 logits
            all_logits = all_logits_batch[i].unsqueeze(0)  # 形状: (1, H, W, S)

            # 3. 调用新的 select_action_flat 函数
            # 确保你已经将 select_action_flat 函数定义或导入
            selected_cell, selected_state, log_prob = select_action_flat(
                grid, all_logits, fiber_block_library,
...
def calculate_batched_loss_with_entropy(batch_log_probs, batch_rewards, all_state_probs_history,
    ...
    **MODIFIED to perform discounted reward calculation directly on the GPU.**
    ...
    final_combined_loss = avg_policy_loss

    return final_combined_loss, avg_policy_loss.item(), avg_entropy.item()


def select_action_flat(grid, all_logits, fiber_block_library, epsilon=0.1, greedy=False):
    """
    为单头网络设计的动作选择函数。

    参数:
        grid (list of lists): 当前游戏网格。
        all_logits (torch.Tensor): 网络输出的原始 logits。形状: (1, H, W, S)
        fiber_block_library (dict): 模块库。
        epsilon (float): Epsilon-greedy 策略的探索率。
        greedy (bool): 是否强制使用贪婪策略。

    返回:
        tuple: (选择的单元格, 选择的状态, log_prob)
    """
    b, h, w, n_s = all_logits.shape
    logits_squeezed = all_logits.squeeze(0)  # 形状: (H, W, S)

    # 1. 创建一个 mask 来屏蔽所有无效动作
    # 无效动作包括：(a) 在已坍缩的单元格中选择 (b) 选择一个不被允许的 state
    mask = torch.full_like(logits_squeezed, float('-inf'))
    state_keys = list(fiber_block_library.keys())

    has_valid_action = False
    for r in range(h):
        for c in range(w):
            if len(grid[r][c]) > 1:  # 如果单元格尚未坍缩
                for state in grid[r][c]:
                    s_idx = state_keys.index(state)
                    mask[r, c, s_idx] = 0.0  # 标记为有效动作
                    has_valid_action = True

    if not has_valid_action:
        return None, None, None

    # 2. 应用 mask 并计算有效动作的概率
    masked_logits = logits_squeezed + mask
    flat_logits = masked_logits.view(-1)  # 展平为一维向量

    # 过滤掉 -inf 的 logits，只在有效动作上计算 softmax
    valid_indices = torch.isfinite(flat_logits)
    valid_flat_logits = flat_logits[valid_indices]

    # 3. 根据策略选择一个动作
    if greedy or random.random() > epsilon:
        # 贪婪或利用
        probs = F.softmax(valid_flat_logits, dim=0)
        chosen_flat_idx = torch.multinomial(probs, 1).item()
    else:
        # 探索
        chosen_flat_idx = random.randrange(len(valid_flat_logits))

    # 4. 计算 log_prob 用于训练
    log_probs = F.log_softmax(valid_flat_logits, dim=0)
    log_prob = log_probs[chosen_flat_idx]

    # 5. 将选择的扁平索引转换回 (r, c, state)
    original_indices = torch.where(valid_indices)[0]
    original_chosen_idx = original_indices[chosen_flat_idx]

    # 使用整数除法和取模运算来恢复索引
    r_chosen = original_chosen_idx // (w * n_s)
    c_chosen = (original_chosen_idx % (w * n_s)) // n_s
    s_idx = original_chosen_idx % n_s

    selected_state_name = state_keys[s_idx]

    return (r_chosen.item(), c_chosen.item()), selected_state_name, log_prob
...
def main():
    ...
    network = WFC_RL_Net_Flat(grid_size[0], grid_size[1], num_states).to(device)
