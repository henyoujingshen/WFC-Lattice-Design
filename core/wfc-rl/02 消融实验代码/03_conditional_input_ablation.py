...
class WFC_RL_Net_V2(nn.Module):
    """
    一个条件化的强化学习网络，用于波函数坍缩（WFC）。

    该网络将网格的当前状态（约束）和目标弹性模量（K矩阵）作为输入。
    它使用 FiLM (Feature-wise Linear Modulation) 层将弹性模量信息
    注入到网络的中间层，从而使网络的决策过程以目标 K 矩阵为条件。

    网络有两个输出头：
    1.  cell_head: 预测在哪个位置（cell）进行下一步操作。
    2.  state_head: 预测在选定的位置上，应该选择哪种模块（state）。
    """
    ...
    def __init__(self, grid_height, grid_width, num_states, num_angles=6,
        ...
        参数:
            ...
            grid_width (int): 网格宽度。
            num_states (int): 可选模块（状态）的总数。
            num_angles (int): 目标弹性模量向量的维度（默认为6）。
            cnn_channels (int): CNN 中间层的通道数。
            elastic_encoded_dim (int): 弹性模量编码后的特征维度。
            fused_feature_dim (int): 融合特征的维度。
        """
        super().__init__()
        ...
        # --- 3. FiLM (Feature-wise Linear Modulation) 层 ---
        # 这个线性层将编码后的弹性模量信息转换为调制参数 (gamma 和 beta)
    ...
    def forward(self, grid_constraints, elastic_modulus):
        ...
        参数:
            grid_constraints (torch.Tensor): 网格约束张量。形状: (B, H, W, S)
            elastic_modulus (torch.Tensor): 目标弹性模量向量。形状: (B, num_angles)

        返回:
            tuple[torch.Tensor, torch.Tensor]:
                - cell_logits (torch.Tensor): 每个单元格的原始 logits。形状: (B, H, W)
                - state_probs (torch.Tensor): 每个单元格中每个状态的概率。形状: (B, H, W, S)
        """
        # 1. 处理网格输入
        # 形状从 (B, H, W, S) -> (B, S, H, W) 以适应 Conv2d
        x_grid_spatial = grid_constraints.permute(0, 3, 1, 2).float()
        x_grid_features = self.cnn(x_grid_spatial)

        # 2. 处理条件输入 (弹性模量)
        # 注意: 这里的归一化值是硬编码的，更好的做法是从您的数据集中计算 min/max
        ...
        # 3. FiLM 调制
        # 生成 gamma (缩放) 和 beta (平移) 参数
        film_params = self.elastic_to_film_params(x_elastic_encoded)
        gamma, beta = torch.chunk(film_params, 2, dim=1)

        # 调整 gamma 和 beta 的形状以匹配特征图 (B, C) -> (B, C, 1, 1)
        # 这样它们就可以通过广播机制应用到特征图的每个空间位置上
        gamma = gamma.unsqueeze(-1).unsqueeze(-1)
        beta = beta.unsqueeze(-1).unsqueeze(-1)

        # 应用 FiLM: element-wise 的仿射变换
        # (gamma + 1) 的做法是为了让变换在训练初期接近于恒等变换，有助于稳定训练
        modulated_grid_features = x_grid_features * (gamma + 1) + beta

        # 4. 融合和输出
        fused_output = self.fusion_conv(modulated_grid_features)

        # 5. 从两个头获取输出
        # cell_logits: (B, 1, H, W) -> (B, H, W, 1) -> (B, H, W)
        cell_logits = self.cell_head(fused_output).permute(0, 2, 3, 1).squeeze(-1)

        # state_logits: (B, S, H, W) -> (B, H, W, S)
        state_logits = self.state_head(fused_output).permute(0, 2, 3, 1)

        # 6. 计算状态概率
        # 在最后一个维度（状态维度）上应用 softmax
...
def run_batched_episodes(network, fiber_block_library, goal_K_tensor, initial_grid,
    ...
    for step in range(max_steps_per_episode):
        ...
        # 1. 获取当前批次的大小 (即活跃环境的数量)
        num_active_envs = batch_constraints.shape[0]

        # 2. 将 goal_K_tensor 从 (1, num_angles) 扩展到 (num_active_envs, num_angles)
        #    这样每个环境的输入都收到了相同的目标 K 矩阵作为条件
        #    .expand() 是一个高效的操作，它不会实际复制数据
        goal_K_tensor_batched = goal_K_tensor.expand(num_active_envs, -1)  # -1 表示保持该维度大小不变

        # 3. 使用两个参数调用网络的前向传播
        cell_logits_batch, state_probs_batch = network(batch_constraints, goal_K_tensor_batched)
...
def main():
    ...
    # 假设您的 K 向量有 6 个值
    NUM_STIFFNESS_DIRECTIONS = 6

    # 实例化新的条件化网络
    network = WFC_RL_Net_V2(
        grid_height=grid_size[0],
        grid_width=grid_size[1],
        num_states=num_states,
        num_angles=NUM_STIFFNESS_DIRECTIONS  # <--- 新增参数
    ).to(device)
